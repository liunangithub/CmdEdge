...
def train_model(treeDic, x_test, x_train, args, iter, fold_count= "default"):
    ...
    name = "best_pre_" + datasetname + "_4unsup" + ".pkl"
    ...
    classfy_model = Classfier(64 * 3, 64, 4).to(args.device)
    ...
    for epoch in range(args.n_epochs):
        traindata_list, testdata_list = loadData(args.datasetname, treeDic, x_train, x_test, args.TDdroprate, args.BUdroprate)
        train_loader = DataLoader(traindata_list, batch_size=args.batchsize, shuffle=True, num_workers=4)
        test_loader = DataLoader(testdata_list, batch_size=args.batchsize, shuffle=True, num_workers=4)
        avg_loss = []
        avg_acc = []
        batch_idx = 0
        tqdm_train_loader = tqdm(train_loader)
        ...
        for Batch_data in tqdm_train_loader:
            Batch_data.to(args.device)
            _, Batch_embed = model.TDrumorGCN(Batch_data,edge_weight=None,edge_infer=False)
            out_labels = classfy_model(Batch_embed, Batch_data)
        ...
        for Batch_data in tqdm_test_loader:
            Batch_data.to(args.device)
            _, Batch_embed = model.TDrumorGCN(Batch_data,edge_weight=None,edge_infer=False)
            val_out = classfy_model(Batch_embed, Batch_data)
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_accs), np.mean(temp_val_F1), np.mean(temp_val_F2),
                       np.mean(temp_val_F3), np.mean(temp_val_F4), model, 'RDEA_' + str(fold_count) + '_', args.datasetname)
        ...
        if epoch >= 199:
            ...
            F2 = early_stopping.F4
        if early_stopping.early_stop:
            print("Early stopping")
            accs = early_stopping.accs
            F1 = early_stopping.F2
            F3 = early_stopping.F3
            F4 = early_stopping.F1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasetname', type=str, default="Twitter15", metavar='dataname',
                        help='dataset name')
    ...
    parser.add_argument('--edge_infer_td', action='store_true', default=True,
    ...
    for iter in range(args.iterations):
        ...
        fold_tests, fold_trains = load5foldData(args.datasetname, seed=args.seed)  #create data,test and train,each has 5 array
