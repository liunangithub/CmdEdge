...
def main(obj):
    ...
    def loadEid(event,id,y):
        ...
        if len(event) < 2:
            return None
        if len(event)>1:
            nodeindex = np.array(list(event.keys()))
            ...
            np.savez( os.path.join(cwd, 'data/'+obj+'graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y, nodeindex=nodeindex)
            return None
...
if __name__ == '__main__':
    obj= "Twitter15" #Twitter16
    savepath = os.path.join(cwd,'data/'+obj+'graph')

    isExists = os.path.exists(savepath)
    if not isExists:
        os.makedirs(savepath)
        print(savepath+'reate successfully!')
    main(obj)
